import {
  MarkerContextMenu,
  MARKER_EDITOR_CONTEXT_MENU_EVENTS,
} from "./controls/context.menu";
...
import {
  MarkerEditorControlPanel,
  MARKER_EDITOR_CONTROL_PANEL_EVENTS,
} from "./controls/marker.control.panel";
...
export class MapMarkerEditor {
  constructor(map) {
    ...
    this.control = new MarkerEditorControlPanel(
      map,
      "#marker-editor-control-panel",
    );
  ...
  apply() {
    ...
    this.map.addInteraction(
      new Translate({ features: this.select.getFeatures() }),
    );
  ...
  bindEvents() {
    ...
    this.map.on(MARKER_EDITOR_CONTEXT_MENU_EVENTS.REMOVE_MARKER, () => {
      easeMarkerOut(
        ...
        () => {
          this.serializer
            .removeFeature(this.selectedMarker.getId())
            .then((_) => {
    ...
    this.control.on(
      MARKER_EDITOR_CONTROL_PANEL_EVENTS.MARKER_HAS_MODIFIED,
      ({ marker }) => {
        this.serializer.serializeFeature(marker);
      },
    );
